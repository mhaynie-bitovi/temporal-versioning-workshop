...
5. **Wait for v1.0 to drain.** After setting v2.0 as Current, no new workflows are routed to v1.0. But existing PINNED workflows are still running there.
   ...
   **Check drain status** using either method:
   ...
   - **Web UI:** Open [http://localhost:8233](http://localhost:8233) and navigate to **Workers > Deployments**. Click on the `valet` deployment to see per-version status. When 1.0 shows as drained, it has no remaining workflows.
