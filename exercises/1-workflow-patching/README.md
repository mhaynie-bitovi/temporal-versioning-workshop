...
In this exercise, you'll learn to catch that with replay testing and fix it with workflow patching.
...
2. Briefly examine `valet/valet_parking_workflow.py`. Note the sequence of steps:
   1. `request_parking_space`
   2. `move_car` (to parking space)
   3. `sleep` (simulate the owner's trip)
   4. `move_car` (back)
...
7. Briefly open `history/valet-CA-1ABC123-history.json` and skim the exported history. Each entry in the `events` array represents something the workflow did - starting activities, recording results, firing timers, etc. This is the sequence of commands the replayer will compare against your code.
...
4. Open the Temporal Web UI at [http://localhost:8233](http://localhost:8233). Find both workflow executions and open their detail pages. They should both complete within about 30 seconds.
